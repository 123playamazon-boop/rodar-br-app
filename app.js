...
const Guide = (function(){
  ...
  function fieldHTML(f){
    ...
    if(f.help){
      ...
      const docUrl=(g.data.prodoc||"").trim()||PRODOC;
      const isDoc=["afflink2","afflink3","afflink6","codenames","bg_track","bg_conv"].indexOf(f.id)>-1;
      const prodUrl=acc?("https://backoffice.buygoods.com/?a="+encodeURIComponent(acc)):"https://backoffice.buygoods.com/campaigns";
      const url=f.helpUrl||(isDoc&&docUrl?docUrl:(prodUrl+(f.helpAnchor||"")));
      const lbl=f.helpLabel||(isDoc&&docUrl?"Abrir a doc do produtor ↗":(acc?("Abrir o produto ("+acc+") ↗"):"Abrir Affiliate Hub ↗"));
